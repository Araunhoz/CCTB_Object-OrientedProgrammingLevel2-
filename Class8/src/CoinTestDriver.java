...
public class CoinTestDriver {
	...
	static void findCoin(HashSet<Coin> bagOfCoins, Coin coin){
		if(bagOfCoins.contains(coin)) {
			System.out.println("There is a coin " + coin.getDenomination() + " in little bag");
			
		}else {
			System.out.println(" No coin " + coin.getDenomination() + " in little bag");
	...
	static void displayBagContents(HashSet<Coin> bagOfCoins){
		...
		if(bagOfCoins.isEmpty()) {
			...
			System.out.println("There are no coins in little bag");	
		
		} else {
			
			System.out.println("There are " + bagOfCoins.size()+ " coins in little bag");  
		}
		}
}
